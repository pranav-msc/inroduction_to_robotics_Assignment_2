...
while (c == 'y'):
    ...
    c=input("Enter y to continue to add more rotations: ")

answer = rotation_matrix[0]
for i in range(len(rotation_matrix)):
    if(i+1<len(rotation_matrix)):
        answer=np.matmul(answer,rotation_matrix[i+1])

original_coordinates=np.matmul(answer,np.transpose(new_coordinates))
